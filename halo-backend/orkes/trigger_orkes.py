...
def load_test_transcript(file_path=None, test_index=0):
    """Load a test transcript from a JSON file if provided"""
    if not file_path:
        return DEFAULT_TRANSCRIPT, DEFAULT_CALL_ID
    
    try:
        with open(file_path, 'r') as f:
            data = json.load(f)
            
            # Check if data is a list (array of test cases)
            if isinstance(data, list):
                if test_index >= len(data):
                    print(f"Warning: Test index {test_index} out of range. Using first test case.")
                    test_index = 0
                
                test_case = data[test_index]
                transcript = test_case.get("transcript", DEFAULT_TRANSCRIPT)
                call_id = test_case.get("id", DEFAULT_CALL_ID)
                
                print(f"Using test case {test_index}: ID={call_id}")
            ...
            # If it's a single object with transcript field
                ...
                call_id = data.get("id", DEFAULT_CALL_ID)
                return transcript, call_id
            
            # Otherwise use default
            else:
                print("Warning: Unexpected JSON format. Using default transcript.")
                return DEFAULT_TRANSCRIPT, DEFAULT_CALL_ID
...
def get_jwt_token():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print("\nPossible issues:")
            print("1. Invalid key ID or secret")
            print("2. Key doesn't have the necessary permissions")
            print("3. The Orkes server URL is incorrect")
            return None
    except Exception as e:
        ...
        if response.status_code == 200:
            try:
                if response.headers.get('Content-Type', '').startswith('application/json'):
                    ...
                    # Plain text response with workflow ID
...
def main():
    ...
    parser.add_argument("--call-id", "-c", default=None, 
                        help="Call ID to use for the workflow (overrides test file ID)")
    ...
    parser.add_argument("--test-index", "-i", type=int, default=0,
                        help="Index of the test case to use (if test file contains multiple cases)")
    ...
    # Determine which transcript and call ID to use
    transcript = args.transcript
    call_id = args.call_id
    
    if not transcript:
        # Load from test file or use default
        transcript, file_call_id = load_test_transcript(args.test_file, args.test_index)
        # Only use call ID from file if not explicitly provided
        if not call_id:
            call_id = file_call_id
    
    # If call_id is still None, use default
    if not call_id:
